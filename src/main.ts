...
room.onRoomLink = (url: string) => {
	...
	Logger.logEvent(
		'superuser-password',
		userConfiguration.superuserPassword,
		new LoggerStyles('blue')
	);
...
room.onPlayerJoin = (player: Player) => {
	...
	if (!PlayerRecord.isValidName(player.name)) {
		room.kickPlayer(
			player.id,
			LanguageProvider.get('Invalid name.'),
			false
		);
		return;
	}
	...
	AuthSystem.authenticate(player.auth, player.name).then(async (record) => {
		if (record) {
			...
			room.kickPlayer(
				player.id,
				LanguageProvider.get('Name taken.'),
				false
			);
	});
};
...
room.onPlayerLeave = (player: Player) => {
	...
	if (AdminManager.getAdminCount() == 0) AdminManager.giveRandomAdmin();
};
...
room.onStadiumChange = async (newStadiumName: string, byPlayer: Player) => {
	...
	if (byPlayer && !(await PlayerDB.findByName(byPlayer.name)).isSuperUser) {
		room.sendAnnouncement(
			LanguageProvider.get(
				'Use !x3 !x1 or !penalties to change the map.'
			),
			byPlayer.id,
			colors.error
		);
...
room.onPlayerAdminChange = (changedPlayer: Player, byPlayer: Player) => {
	Logger.logEvent(
		'adminchange: ' + (changedPlayer.admin ? 'on' : 'off'),
		changedPlayer.name,
		new LoggerStyles('orange')
	);

	if (AdminManager.getAdminCount() == 0) AdminManager.giveRandomAdmin();
};

room.onPlayerKicked = async (
	kickedPlayer: Player,
	reason: string,
	ban: boolean,
	byPlayer: Player
	...
	Logger.logEvent(
		'playerkicked: ' + (ban ? 'ban' : 'kick'),
		kickedPlayer.name,
		new LoggerStyles('red')
	);
	...
	if (ban) {
		if ((await PlayerDB.findByName(kickedPlayer.name))?.isSuperUser) {
			if (byPlayer && byPlayer.id != kickedPlayer.id)
				room.kickPlayer(
					byPlayer.id,
					LanguageProvider.get('You cant ban that player.'),
					false
				);
...
room.onPlayerChat = (player: Player, message: string) => {
	Logger.logEvent(
		'playerchat',
		player.name + ': ' + message,
		new LoggerStyles('skyblue')
	);

	if (SpamFilter.match(message)) {
		room.kickPlayer(
			player.id,
			LanguageProvider.get('Spamming the room is forbidden.'),
			!player.admin
		);
		return false;
	}
	...
	PlayerDB.findByName(player.name).then((record) => {
		if (record.isMuted()) {
			room.sendAnnouncement(
				LanguageProvider.get('You are silenced.'),
				player.id,
				colors.error
			);
			return;
		...
		if (message.charAt(0) == '#' && (player.admin || record.isSuperUser)) {
			Chat.sendAdmins(
				`[admin chat] ${player.name}: ${message.substring(1)}`
			);
			return;
		}
		...
		if (message.charAt(0) == '!') {
			...
			if (
				!GameCommandFactory.process(
					command.command,
					new GameCommandContext(player, command.args, record, room)
				)
			)
				room.sendAnnouncement(
					LanguageProvider.get('Command not found.'),
					player.id,
					colors.error
				);
			return false;
...
room.onGameTick = () => {
	...
	const players = room
		.getPlayerList()
		.filter((p) => p.team != TeamID.Spectators);

	for (let player of players) {
		const playerRadius: number = room.getPlayerDiscProperties(
			player.id
		).radius;
		const distanceToBall: number = Math.sqrt(
