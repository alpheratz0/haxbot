...
export class PlayerDB {
	...
	static async connect(dbName: string): Promise<void> {
		return new Promise((resolve, reject) => {
			...
			request.onupgradeneeded = (ev) => {
				const store = request.result.createObjectStore('players', {
					keyPath: 'auth'
				});
				store.createIndex('name', 'name', { unique: true });
				store.createIndex('names', 'names', {
					unique: true,
					multiEntry: true
				});
				...
				store.createIndex('undefeated', 'undefeated', {
					unique: false
				});
	...
	static async find(auth: string): Promise<PlayerRecord | null> {
		return new Promise((resolve, reject) => {
			...
			request.onsuccess = (event) =>
				resolve(
					request.result
						? PlayerRecord.fromPartial(request.result)
						: null
				);
	...
	static async exists(auth: string): Promise<boolean> {
		return new Promise<boolean>((resolve, reject) => {
			...
			request.onerror = (evt) =>
				reject('error while checking player existence');
		});
	}
		...
		return new Promise<PlayerRecord[]>((resolve, reject) => {
			...
			request.onsuccess = (evt) => {
				...
				if (cursor && items.length < maxResults) {
					...
					if (func(player)) items.push(player);
					cursor.continue();
				} else resolve(items);
			};
	...
	static async getRecordsConnected(): Promise<PlayerRecord[]> {
		return await Promise.all(
			room
				.getPlayerList()
				.map((player) => PlayerDB.findByName(player.name))
		);
	}
