...
export class Chat {
    private static readonly rainbowColors: number[] = [0xff0000, 0xff7f00, 0xffff00, 0x00ff00, 0x0000ff, 0x7a6cad, 0x8B00ff];
    ...
    static rainbow(message: string): void {
        if(this.rainbowSide) {
            ...
            this.rainbowIndex--;
            if(this.rainbowIndex == 0) 
                this.rainbowSide = true;
        }
    
        room.sendAnnouncement(message, undefined, this.rainbowColors[this.rainbowIndex]);
    }
}