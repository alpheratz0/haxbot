...
export class Table {
	...
	addRow(row: (string | number)[]): void {
		this.rows.push(
			row.map((v) => (typeof v == 'number' ? v.toString() : v))
		);
	}
	...
	display(): void {
		...
		for (let index: number = 0; index < this.rows[0].length; index++) {
			...
			for (let p = 0; p < this.rows.length; p++)
				if (
					Table.measurer.measure(this.rows[p][index]) + 30 >
					separators[index]
				)
					separators[index] =
						Table.measurer.measure(this.rows[p][index]) + 30;
		...
		for (let index: number = 0; index < this.rows.length; index++) {
			...
			for (
				...
				if (rowIndex != 0)
					separators[rowIndex] =
						separators[rowIndex] +
						(separators[rowIndex - 1] -
							Table.measurer.measure(
								Table.measurer.padRight(
									row[rowIndex - 1],
									separators[rowIndex - 1]
								)
							));
				result += Table.measurer.padRight(
					row[rowIndex],
					separators[rowIndex]
				);
			}
