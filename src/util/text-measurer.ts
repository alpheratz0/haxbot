export class TextMeasurer {
    ...
    */
    ...
    padLeft(text: string, width: number, char?: string): string {
        ...
        const longSpacesCount = (longSpaceAvailable / this.measure(" ")) - ((longSpaceAvailable / this.measure(" ")) % 1);
        
        const shortSpaceAvailable = longSpaceAvailable - longSpacesCount * this.measure(" ");
        const shortSpacesCount = (shortSpaceAvailable / this.measure(" ")) - ((shortSpaceAvailable / this.measure(" ")) % 1);

        return text.padStart(longSpacesCount + text.length, ' ').padStart(shortSpacesCount + text.length + longSpacesCount, ' ');
    }
    ...
    padRight(text: string, width: number, char?: string): string {
        ...
        return text.padEnd(longSpacesCount + text.length, ' ').padEnd(shortSpacesCount + text.length + longSpacesCount, ' ');
    }
    ...
    padBoth(text: string, width: number, char?: string): string {
        ...
        return this.padLeft(this.padRight(text, startWidth + spaceAvailable/2, char), startWidth + spaceAvailable, char);
    }
}