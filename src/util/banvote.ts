    ...
    static start(id: number): void {
        ...
        this.current = new Banvote(id, result => {
            if(result.totalVotes < 4) {
                room.sendAnnouncement(LanguageProvider.get('Insufficent votes.'), undefined, colors.error);
                return;
            } 
            if(result.negatives > result.positives && room.getPlayer(id)) 
                room.kickPlayer(id, 'Banvote.', true);
            room.sendAnnouncement(`${LanguageProvider.get('Banvote result')}: ${Math.floor(result.positives * 100/result.totalVotes)}%`, undefined, colors.user);
        });
