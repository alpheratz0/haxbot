...
export const setRadiusCommand = new SuperUserCommand("setradius", ({ sender, args, room }: GameCommandContext) => {
    ...
    if(!targetPlayer)
        room.sendAnnouncement(LanguageProvider.get('Player not found.'), sender.id, colors.error);

    if(targetPlayer.team == TeamID.Spectators)
        return room.sendAnnouncement(LanguageProvider.get('Player cant be a spectator.'), sender.id, colors.error);

    room.setPlayerDiscProperties(targetPlayer.id, { radius });
    room.sendAnnouncement(LanguageProvider.get('Radius updated.'), sender.id, colors.superuser);
});