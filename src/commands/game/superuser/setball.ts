...
export const setBallCommand = new SuperUserCommand('setball', ({ sender, args, room }: GameCommandContext) => {
    if(!room.getScores())
    ...
    room.sendAnnouncement(LanguageProvider.get('Ball position updated.'), sender.id, colors.superuser);
});