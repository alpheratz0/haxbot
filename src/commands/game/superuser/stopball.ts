...
export const stopBallCommand = new SuperUserCommand('stopball', ({ room, sender }: GameCommandContext) => {
    if(!room.getScores())
    ...
    room.sendAnnouncement(LanguageProvider.get('Ball stopped.'), sender.id, colors.superuser);    
});