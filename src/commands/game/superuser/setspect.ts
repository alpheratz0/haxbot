...
export const setSpectCommand = new SuperUserCommand('setspect', ({ room, args, sender }: GameCommandContext) => {
    ...
    if(targetPlayer.team != TeamID.Spectators) 
        return room.sendAnnouncement(LanguageProvider.get('Player must be spectator.'), sender.id, colors.error);

    const spectIds = room.getPlayerList()
                         .filter(p => p.team == TeamID.Spectators && p.id != targetId)
                         .map(p => p.id);

    const newPosition = Math.max(0, Math.min(parseInt(args.shift()) || 0, spectIds.length));
