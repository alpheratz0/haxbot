...
export const mergeCommand = new SuperUserCommand(
	'merge',
	async ({ sender, args, room }: GameCommandContext) => {
		...
		if (!carnetFrom)
			return room.sendAnnouncement(
				LanguageProvider.get('Auth not found on db.'),
				sender.id,
				colors.error
			);
