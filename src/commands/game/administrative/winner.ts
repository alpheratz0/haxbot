...
export const winnerCommand = new AdministrativeCommand('winner', ({ room, sender }: GameCommandContext) => {
    const winnerTeamName = Game.lastWinnerId == TeamID.Red ? "red" : "blue";
    room.sendAnnouncement(LanguageProvider.get('Last team winner: ') +winnerTeamName, sender.id, colors.admin);
})