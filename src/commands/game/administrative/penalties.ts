...
export const penaltiesCommand = new AdministrativeCommand('penalties', ({ sender, room }: GameCommandContext) => {
    if(room.getScores())
        return room.sendAnnouncement(LanguageProvider.get('Game cannot be started.'), sender.id, colors.error);
