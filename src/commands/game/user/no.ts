...
export const noCommand = new UserCommand('no', ({ room, record, sender }: GameCommandContext) => {
    switch(Banvote.vote(record.auth, Vote.Negative)) {
        case VoteResult.Success:
            room.sendAnnouncement(LanguageProvider.get('Someone voted no.'), undefined, colors.user, "italic");
        break;
