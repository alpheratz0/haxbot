...
export const celebrationCommand = new UserCommand("celebration", ({ sender, args, room, record }: GameCommandContext) => {
    record.scoreMessage = args.join(" ").trim() || null;
    room.sendAnnouncement(LanguageProvider.get('Celebration message updated.'), sender.id, colors.user);
