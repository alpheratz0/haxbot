...
export const donateCommand = new UserCommand('donate', ({ args, sender, room, record }: GameCommandContext) => {
    ...
    if(targetId == sender.id)
        return room.sendAnnouncement(LanguageProvider.get('Cant donate to yourself.'), sender.id, colors.error);

    if(ammount > record.cash || ammount <= 0)
        return room.sendAnnouncement(LanguageProvider.get('Invalid ammount to donate.'), sender.id, colors.error);
