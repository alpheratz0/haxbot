...
export const welcomeCommand = new UserCommand('welcome', ({ record, args, sender, room }: GameCommandContext) => {
    record.welcomeMessage = args.join(" ").trim() || null;
    room.sendAnnouncement(LanguageProvider.get('Welcome message updated.'), sender.id, colors.user);
