...
export class Game {
	...
	static getWinners(): string[] {
		...
		for (let goalData of this.goalTimeline) {
			if (this.getWinnerTeam() == TeamID.Red)
				winners.push(...goalData.redTeam);
	...
	static getGkWinner(): string {
		const winnerGks = this.goalTimeline.reduce<
			{ auth: string; repetitions: number }[]
		>((prev, current) => {
			const gkWinner =
				this.getWinnerTeam() == TeamID.Red
					? current.redGk
					: current.blueGk;
	...
	static touch(player: Player): void {
		...
		if (
			!last ||
			last.player.name != player.name ||
			last.player.team != player.team
		) {
			this.touchTimeline.push(
				new PlayerTouch(player, room.getScores().time)
			);
			return;
		}
	...
	static async goal(team: number): Promise<void> {
		...
		const scorer =
			this.touchTimeline.filter(
				(p) => p.player.team == team && p.time > score.time - 2.5
			)[0] || this.touchTimeline.pop();
		const assistant =
			scorer && scorer.player.team == team
				? this.touchTimeline
						.filter(
							(p) =>
								p.player.team == team &&
								p.player.name != scorer.player.name &&
								p.time < scorer.time &&
								p.time > scorer.time - 3
						)
						.pop()
				: null;

		const goalTime = Math.floor(score.time / 2);
		const goalTimeMessage =
			goalTime <= 90 ? `[ ${goalTime}' ⚽ ] ` : `[ 90+ ⚽ ] `;
		...
		if (scorer) {
			...
			if (scorer.player.team == team) {
				goalMessage +=
					scorerCarnet.scoreMessage ??
					LanguageProvider.get('Goaaaal by ') + scorer.player.name;
				scorerCarnet.goals++;
				scorerCarnet.cash += 50;
				await PlayerDB.update(scorerCarnet);

				if (assistant) {
					const assistantCarnet = await PlayerDB.findByName(
						assistant.player.name
					);
					goalMessage +=
						' / ' +
						LanguageProvider.get('Assist') +
						': ' +
						assistant.player.name;
				...
				scorerCarnet.cash -= 50;
				goalMessage +=
					LanguageProvider.get('Own goal by') +
					' ' +
					scorer.player.name;
				room.sendAnnouncement(
					goalMessage,
	...
	static async end(): Promise<void> {
		if (this.redScore != this.blueScore) {
			...
			room.sendAnnouncement(
				`${LanguageProvider.get('Consecutive wins')}: ${
					this.consecutiveWins
				}`,
				undefined,
				colors.user,
			...
			const losersAuths = this.getLosers().filter(
				(auth) => !winnersAuths.some((winauth) => winauth == auth)
			);
